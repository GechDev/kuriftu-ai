...
class MarketEnvironment:
    ...
    def _initialize_products(self):
        """Kuriftu resort services — aligned with NEXORA demo catalog."""
        resort_catalog = [
            ("Signature Spa Ritual (90m)", "Spa", 120.0),
            ("Lakehouse Tasting Menu", "Dining", 95.0),
            ("Sunset Boat Ride", "Experiences", 65.0),
            ("Deluxe Lake View (nightly)", "Rooms", 260.0),
            ("Executive Suite (nightly)", "Rooms", 400.0),
            ("Private Wine Cellar Dinner", "Dining", 185.0),
            ("Lagoon Kayak Experience", "Experiences", 45.0),
            ("Couples Massage (60m)", "Spa", 95.0),
            ("Helicopter Scenic Tour", "Experiences", 320.0),
        ]
        n = min(self.num_products, len(resort_catalog))
        products = []
        for i in range(n):
            name, category, base = resort_catalog[i]
            jitter = random.uniform(-4.0, 4.0)
            base_price = round(max(25.0, base + jitter), 2)
            products.append({
                'id': i,
                'name': name,
                'base_price': base_price,
                'current_price': base_price,
                'cost': round(base_price * random.uniform(0.38, 0.52), 2),
                'stock': random.randint(40, 120),
                'demand_elasticity': round(-(1.15 + random.random() * 0.75), 3),
                'category': category,
                'quality': round(random.uniform(0.72, 0.98), 3),
                'seasonality': round(random.uniform(0.85, 1.18), 3),
            })
        return products
